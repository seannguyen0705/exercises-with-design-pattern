...
import { SearchBook } from "../../Solid/1.Srp/SearchBook";
...
describe("Library", () => {
  ...
  test("should return an SeachBook object of library", () => {
    expect(library.getSearchBook()).toBeInstanceOf(SearchBook);
  });
});

describe("SearchBox", () => {
  let books: Book[];
  let search: SearchBook;
  ...
  test("should return books by publication year", () => {
    ...
    expect(booksByYear.every((book) => book.publicationYear === 2022));
  });
